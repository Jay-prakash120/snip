...
const COMMANDS = ['cat', 'edit', 'ls', 'rm', 'sync', 'export', 'config', 'completion', 'help', 'clear', 'exit', 'quit'];
      ...
      try {
        switch (command) {
          ...
          case 'completion':
            ...
            break;
          default:
            // If user typed a snippet name directly: e.g. "my_snippet()"
            if (command) {
              const msgIdx = rest.indexOf('-m') !== -1 ? rest.indexOf('-m') : rest.indexOf('--message');
              const langIdx = rest.indexOf('-l') !== -1 ? rest.indexOf('-l') : rest.indexOf('--lang');
              const options = {
              ...
              await createCommand(command, options);
            } else {
